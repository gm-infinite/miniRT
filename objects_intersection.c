...
double	cylinder_side_intersection(t_ray r, t_cylinder cy)
{
    ...
    b = 2 * (cy.ray_t_pos.x * r.direction.x + cy.ray_t_pos.z * r.direction.z);
    discriminant = b * b - 4 * a
        * ((cy.ray_t_pos.x * cy.ray_t_pos.x)
            + (cy.ray_t_pos.z * cy.ray_t_pos.z) - cy.radius * cy.radius);
    r.t = -1;
    if (a > T_ZERO_THRESHOLD && discriminant >= 0)
    {
        tt = (-b - sqrt(discriminant)) / (2 * a);
        if (tt > T_ZERO_THRESHOLD
            && fabs(cy.ray_t_pos.y + tt * r.direction.y) <= cy.h / 2)
            return (tt);
        tt = (-b + sqrt(discriminant)) / (2 * a);
        if (tt > T_ZERO_THRESHOLD
            && fabs(cy.ray_t_pos.y + tt * r.direction.y) <= cy.h / 2)
            ...
            t_plane cap_plane, double *current_t)
{
    ...
    if (t_plane <= T_ZERO_THRESHOLD)
        return (0);
    x = cy.ray_t_pos.x + t_plane * ray_m.direction.x;
    z = cy.ray_t_pos.z + t_plane * ray_m.direction.z;
		...
		int hit_type)
{
	...
	to_hit = point_substract(v3_mult(t, ray.direction), cy.origin);
